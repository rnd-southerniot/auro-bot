...
`http://192.168.68.110`, DHCP-reserved; override with `$NAVBOT_CAMERA_URL`).

```bash
CAM="${NAVBOT_CAMERA_URL:-http://192.168.68.110}"
echo "camera: $CAM"
# 1) Liveness/health JSON (fps, motion, rssi, uptime, version):
curl -s --max-time 6 "$CAM/status" | python3 -m json.tool 2>/dev/null \
  || { echo "NO /status — camera unreachable (powered? on AP 'Auro'?)"; exit 1; }
...
the XIAO is powered, joined AP "Auro," and still at the reserved address
(`ip neigh show 192.168.68.110` should show MAC `8c:bf:ea:8e:65:04`).
